...
export async function generateInvoicePDF(data) {
  ...
  page.drawText('INVOICE', { x: width - 180, y, size: 26, font: serifBold, color: charcoal })
  y -= 18
  page.drawText('Let a violin speak for your celebration', { x: 50, y, size: 9, font: sans, color: taupe })

  y -= 50
  page.drawText('Invoice to:', { x: 50, y, size: 11, font: sansBold, color: charcoal })
  page.drawText(`${firstName} ${lastName}`, { x: 130, y, size: 11, font: sans, color: charcoal })
  page.drawText('Invoice#', { x: 350, y, size: 11, font: sans, color: charcoal })
  page.drawText(invoiceNumber, { x: width - 130, y, size: 11, font: sans, color: charcoal })

  y -= 20
  page.drawText('Date', { x: 350, y, size: 11, font: sans, color: charcoal })
  page.drawText(invoiceDate, { x: width - 130, y, size: 11, font: sans, color: charcoal })
  ...
  y -= 22
  page.drawText('-Advance payment paid', { x: 50, y, size: 10, font: sans, color: charcoal })
  page.drawText(formatMoney(advancePayment), { x: width - 150, y, size: 10, font: sans, color: charcoal })

  y -= 15
  page.drawLine({ start: { x: 50, y }, end: { x: width - 50, y }, thickness: 1, color: taupe })

  y -= 40
  page.drawText('BALANCE', { x: width - 250, y, size: 14, font: sansBold, color: charcoal })
  page.drawText(formatMoney(balance), { x: width - 150, y, size: 14, font: sansBold, color: charcoal })
  y -= 5
  page.drawLine({ start: { x: width - 250, y }, end: { x: width - 50, y }, thickness: 1, color: charcoal })
  ...
  y -= 15
  page.drawText('Thanks for choosing us!', { x: 50, y, size: 12, font: serif, color: charcoal })
